...
def getFile():
    ...
    print('Writing file...')
    open('us_confirmed_deaths.csv','wb').write(response.content)
...
def createCSV():
    print("us_confirmed_deaths.csv (base file) created!")
    getFile()
    #init data
    data=pd.read_csv('us_confirmed_deaths.csv', delimiter = ',')
    #drop extra columns
